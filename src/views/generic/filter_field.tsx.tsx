...
export function FilterMultiSelectField(props: Props) {
  ...
  function includesId(_value: string): boolean {
    return !!value.find((id) => id === _value);
  }

  function toggleSelection(_value: string) {
    if (includesId(_value)) {
      return props.onChange(value.filter((id) => id !== _value));
    }
    return props.onChange(value.concat([_value as never]));
  }
